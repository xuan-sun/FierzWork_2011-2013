...
int main(int argc, char* argv[])
{
  ...
  TFile f(TString::Format("Octet_%i_ssDataHist_%s_radialCut_%i-%imm_endpointCorr_opt1.root", octNb, TYPE, RADIALCUTLOW, RADIALCUTHIGH), "RECREATE");
...
vector < vector < TH1D* > > CreateRateHistograms(vector <TChain*> runsChains)
{
  ...
  for(unsigned int i = 0; i < runsChains.size(); i++)
  {
    ...
    runsChains[i]->SetBranchAddress("Erecon_corr_r49mm_optimized1", &evt[i]->Erecon);
