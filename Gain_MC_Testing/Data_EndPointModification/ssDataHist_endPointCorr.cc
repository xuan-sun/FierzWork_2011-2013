...
void LoadDataEndpoint(int octNb, TString fileName);
...
int main(int argc, char* argv[])
{
  ...
  // load the endpoint so we can apply an endpoint correction
  LoadDataEndpoint(octNb, Form("../../End_Point_Studies/endPointFits_ssDataHists_type0_%s_Bins_17-65.txt", GEOM));

  // Reads in the octet list and saves the run files indices corresponding to an octet number
  vector < pair <string,int> > octetIndices = LoadOctetList(TString::Format("/home/xuansun/Documents/Analysis_Code/FierzWork_2011-2013/OctetLists/octet_list_%i.dat", octNb));
  ...
  for(unsigned int j = 0; j < runsChains.size(); j++)
  {
    for(unsigned int i = 0; i < runsChains[j]->GetEntriesFast(); i++)
    {
      ...
      if(evt[j]->pid == 1 && evt[j]->type == 0 && evt[j]->Erecon >= 0 && evt[j]->timeFlag == 0)
      {
        if(evt[j]->side == 0)
        {
          rateHistsEast[j]->Fill(evt[j]->Erecon * (782.0 / dataEndPoint));
	  lastEventNumPerChain = i;
        }
        else if(evt[j]->side == 1)
        {
          rateHistsWest[j]->Fill(evt[j]->Erecon * (782.0 / dataEndPoint));
	  lastEventNumPerChain = i;
        }
	totalEventNum++;
      }
  ...
  while(true)
  {
    ...
    if(!infile1.eof())
    {
      bufstream >> evt.indexNb
                ...
                >> evt.lowE
                >> evt.highE;
     if(evt.indexNb == octNb)
     {
       dataEndPoint = evt.endpoint;
     }
    }
