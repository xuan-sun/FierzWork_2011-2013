...
int main(int argc, char* argv[])
{
  ...
  fittedEndpointsForOctets = ReadInOctetEndpoints(Form("optimizedFitWindow_endPointFits_noCorrection_ssDataHists_type0_2011-2012_radialCut_0-49mm_Bins_27-69.txt"), fittedEndpointsForOctets);
  fittedEndpointsForOctets = ReadInOctetEndpoints(Form("optimizedFitWindow_endPointFits_noCorrection_ssDataHists_type0_2012-2013_radialCut_0-49mm_Bins_34-68.txt"), fittedEndpointsForOctets);
  ...
  for(unsigned int i = 0; i < runNumbers.size(); i++)
  {
    ...
    {
//      meanEndpoint = 785.6;	// original 165-645keV fit window
      meanEndpoint = 789.6;	// "optimum" 265-685keV fit window
    }
    // use 2012-2013 octet fitted endpoints; 0 < r < 49mm radial cut.
    else if(runNumbers[i].first >= 80 && runNumbers[i].first <= 121)
    {
//      meanEndpoint = 782.4;	// original 165-645keV fit window
      meanEndpoint = 790.7;	// "optimum" 335-675keV fit window
    }
...
void AddBranchToTreeInFile(TString fName, double fittedOctetEndpoint, double meanEndpoint)
{
  ...
  double ereconCorr_r49mm_optimized1;
  TBranch *b = t->Branch("Erecon_corr_r49mm_optimized1", &ereconCorr_r49mm_optimized1, "ereconCorr_r49mm_optimized1/D");

  for(long int i = 0; i < t->GetEntries(); i++)
  {
    t->GetEntry(i);
    ereconCorr_r49mm_optimized1 = erecon*(meanEndpoint / fittedOctetEndpoint);
    b->Fill();
  }
