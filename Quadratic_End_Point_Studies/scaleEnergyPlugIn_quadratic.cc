...
void AddBranchToTreeInFile(TString fName, double fittedOctetEndpoint, double meanEndpoint)
{
  ...
  double ereconCorr_r49mm_quadratic;
  TBranch *b = t->Branch("Erecon_corr_r49mm_quadratic", &ereconCorr_r49mm_quadratic, "ereconCorr_r49mm_quadratic/D");

  for(long int i = 0; i < t->GetEntries(); i++)
  {
    t->GetEntry(i);
    // E_new = E_recon + Erecon^2 * g^2 where g = sqrt(E_totalMean - E_octetMean)/E_octetMean
    ereconCorr_r49mm_quadratic = erecon + pow(erecon, 2.0) * ((meanEndpoint - fittedOctetEndpoint) / pow(fittedOctetEndpoint, 2.0));
    b->Fill();
  }
