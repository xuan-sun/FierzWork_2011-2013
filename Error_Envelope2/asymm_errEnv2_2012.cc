...
TH1D *hEnvelope2012_top = new TH1D("2012-2013-top", "2012-2013 top", 110, 0, 1100);
TH1D *hEnvelope2012_bot = new TH1D("2012-2013-bot", "2012-2013 bot", 110, 0, 1100);
...
*/
  ...
  errEnv2012_top_2sigma -> SetTitle("Non-linearity Polynomial Variations, 2012-2012");
  ...
  histErecon.push_back(new TH1D("Ce2012-2013", "Erecon = 130, #sigma_{env} = 3.1", 120, -30, 30));
  histErecon.push_back(new TH1D("Sn2012-2013", "Erecon = 368, #sigma_{env} = 2.1", 120, -30, 30));
  histErecon.push_back(new TH1D("BiLow2012-2013", "Erecon = 498, #sigma_{env} = 2.76", 120, -30, 30));
  histErecon.push_back(new TH1D("endpoint", "Erecon = 782, #sigma_{env} = 3.8", 120, -30, 30));
  histErecon.push_back(new TH1D("BiHigh2012-2013", "Erecon = 994, #sigma_{env} = 7.6", 120, -30, 30));

  // Load the converter to get Erecon from a single EQ value.
  cout << "Using following calibration for 2012-2012 geometry to convert Evis to Erecon..." << endl;
...
//  TF1* errEnv2 = errEnv2012_top_2sigma;
  ...
  TF1* errEnvTop1 = errEnv2012_top_1sigma;
  TF1* errEnvTop2 = errEnv2012_top_2sigma;
  TF1* errEnvBot1 = errEnv2012_bot_1sigma;
  TF1* errEnvBot2 = errEnv2012_bot_2sigma;
...
void ProbTwiddleValidity(vector <double> convertedTwiddle, vector <double> energyAxis, TRandom3 *randNum)
{
  ...
  {
    CeErrorBar = convertedTwiddle[Ce_index] / errEnv2012_bot_1sigma->Eval(130.3);
  }
  else if(convertedTwiddle[Ce_index] >= 0)
  {
    CeErrorBar = convertedTwiddle[Ce_index] / errEnv2012_top_1sigma->Eval(130.3);
  }

  if(convertedTwiddle[Sn_index] < 0)
  {
    SnErrorBar = convertedTwiddle[Sn_index] / errEnv2012_bot_1sigma->Eval(368.5);
  }
  else if(convertedTwiddle[Sn_index] >= 0)
  {
    SnErrorBar = convertedTwiddle[Sn_index] / errEnv2012_top_1sigma->Eval(368.5);
  }

  if(convertedTwiddle[Bi1_index] < 0)
  {
    Bi1ErrorBar = convertedTwiddle[Bi1_index] / errEnv2012_bot_1sigma->Eval(498.0);
  }
  else if(convertedTwiddle[Bi1_index] >= 0)
  {
    Bi1ErrorBar = convertedTwiddle[Bi1_index] / errEnv2012_top_1sigma->Eval(498.0);
  }

  if(convertedTwiddle[End_index] < 0)
  {
    EndErrorBar = convertedTwiddle[End_index] / errEnv2012_bot_1sigma->Eval(782);
  }
  else if(convertedTwiddle[End_index] >= 0)
  {
    EndErrorBar = convertedTwiddle[End_index] / errEnv2012_top_1sigma->Eval(782);
  }

  if(convertedTwiddle[Bi2_index] < 0)
  {
    Bi2ErrorBar = convertedTwiddle[Bi2_index] / errEnv2012_bot_1sigma->Eval(993.8);
  }
  else if(convertedTwiddle[Bi2_index] >= 0)
  {
    Bi2ErrorBar = convertedTwiddle[Bi2_index] / errEnv2012_top_1sigma->Eval(993.8);
  }
...
*/
  // weights for 2012-2012 error bars that work best
...
void LoadEnvelopeHistogram_2011()
{
  ...
  while(true)
  {
    ...
    if(!infile1.eof())
    {
      ...
      hEnvelope2011_bot->SetBinContent(hEnvelope2011_bot->FindBin(energy), errorLow);
...
void LoadEnvelopeHistogram_2012()
{
  ...
  while(true)
  {
    ...
    if(!infile1.eof())
    {
      ...
      hEnvelope2012_top->SetBinContent(hEnvelope2012_top->FindBin(energy), errorHigh);
      hEnvelope2012_bot->SetBinContent(hEnvelope2012_bot->FindBin(energy), errorLow);
