...
int main(int argc, char* argv[])
{
  ...
  {	// note that .c_str() converts a std::string into a useable %s in Form(), must like .Data() for TString
    TFile f(Form("/mnt/data2/xuansun/analyzed_files/%s_geom_twiddles/A_0_b_0_baselineHistograms/Hist_noBlind_SimAnalyzed_%s_Beta_paramSet_100_%i_type0.root", GEOM, GEOM, j));
  ...
  {
    TFile f(Form("/mnt/data2/xuansun/analyzed_files/%s_geom_twiddles/A_0_b_inf_baselineHistograms/Hist_noBlind_SimAnalyzed_%s_Beta_paramSet_100_%i_type0.root", GEOM, GEOM, i));
    TH1D* hTemp = (TH1D*)f.Get("Erecon blinded hist");
