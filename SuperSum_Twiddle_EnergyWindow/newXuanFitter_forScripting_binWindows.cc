...
int main(int argc, char* argv[])
{
  if(argc < 4)
  {
    ...
    cout << "(executable) (data file index) (bin low) (bin high)" << endl;
    return 0;
  }
...
//  outfile.open(Form("Twiddles_CorrectBlindingOct2018_newXuanFitter_bFitForSystError_%s_%s_Bins_%i-%i.txt", TYPE, GEOM, fitBinMin, fitBinMax), ios::app);
  outfile << octNb << "\t"
          << avg_mE << "\t"
	  << functionMin << "\t"
	  << ndf << "\t"
